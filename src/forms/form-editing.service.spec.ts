...
describe('FormsEditingService', () => {
  ...
  describe('lockForm', () => {
    it('should lock the form if no other user has locked it', async () => {
      ...
      const result = await formsEditingService.lockForm('form1', { id: 'user1', email: 'user1@example.com' });

      expect(redisService.get).toHaveBeenCalledWith('form-lock:form1');
      expect(redisService.set).toHaveBeenCalledWith('form-lock:form1', { id: 'user1', email: 'user1@example.com' }, 300);
      expect(result).toEqual({ success: true });
    });

    it('should not lock the form if another user has locked it', async () => {
      (redisService.get as jest.Mock).mockResolvedValue({ id: 'user2', email: 'user2@example.com' });
  ...
  describe('keepAlive', () => {
    it('should extend the lock if the same user owns it', async () => {
      (redisService.get as jest.Mock).mockResolvedValue({ id: 'user1', email: 'user1@example.com' });
      (redisService.updateTTL as jest.Mock).mockResolvedValue(null);

      const result = await formsEditingService.keepAlive('form1', 'user1');

      expect(redisService.get).toHaveBeenCalledWith('form-lock:form1');
      expect(redisService.updateTTL).toHaveBeenCalledWith('form-lock:form1', 300);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should throw an error if another user owns the lock', async () => {
      ...
      await expect(formsEditingService.keepAlive('form1', 'user1')).rejects.toThrow('Cannot extend lock, no ownership.');
    });
  });
  ...
  describe('releaseLock', () => {
    ...
    it('should throw an error if another user owns the lock', async () => {
      ...
      await expect(formsEditingService.releaseLock('form1', 'user1')).rejects.toThrow('Cannot release lock, no ownership.');
    });
  });
  ...
  describe('getLockStatus', () => {
    it('should return the current lock status', async () => {
      ...
      const result = await formsEditingService.getLockStatus('form1');
